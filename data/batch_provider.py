...
class BatchProvider():
    ...
    def next_batch(self, batch_size):
        ...
        X_batch = self._post_process_batch(X_batch)
    ...
    def _post_process_batch(self, X_batch):

        if self.convert_to_gray:
            X_batch = np.mean(X_batch, axis=-1, keepdims=True)

        if self.do_augmentations:
            X_batch = self._augmentation_function(X_batch)
    ...
    def _augmentation_function(self, images):
        '''
